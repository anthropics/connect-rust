...
pub trait Checker: Send + Sync + 'static {
    ...
    /// # Errors
    ///
    /// The default implementation returns `Err(ConnectError::unimplemented(_))`,
    /// which is fine for Check-only servers — kubelet and `grpc_health_probe`
    /// don't call Watch. Overrides should return
    /// `Err(ConnectError::not_found(_))` for unknown services.
    fn watch(
        &self,
        service: &str,
